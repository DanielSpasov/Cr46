...
export const command: Command = {
  ...
  run: async (client, message, args) => {
    try {
      ...
      if (!client.music.guilds.get(message.guildId))
        client.music.guilds.set(message.guildId, {
          looping: 0,
      ...
      const voiceChannelID = message.guild.members.cache.get(message.author.id)
        .voice.channelId;
      if (!voiceChannelID)
        return message.channel.send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                `You need to be in a voice channel to use ${dbGuild.prefix}play`
              )
              .setColor("PURPLE"),
          ],
        });
      ...
      if (!args[0])
        return message.channel.send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                `Please provide arguments to the ${dbGuild.prefix}play command`
              )
              .setColor("PURPLE"),
          ],
        });
      ...
      if (typeof videos == "string")
        return message.channel.send({
          embeds: [
            new MessageEmbed().setDescription(videos).setColor("PURPLE"),
          ],
        });
